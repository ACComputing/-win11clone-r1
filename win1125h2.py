...
class Windows11Clone:
    def __init__(self):
        ...
        self.popup_close_binding = None  # For root click binding
    ...
    def close_all_popups(self, exclude=None):
        ...
        # Remove root click binding if no popups left
        if self.popup_close_binding:
            self.root.unbind('<Button-1>', self.popup_close_binding)
            self.popup_close_binding = None

    def setup_outside_click_close(self, popup):
        """Bind a click on the root window to close the popup if click is outside."""
        def check_click(event):
            # If popup no longer exists, unbind
            ...
            # If click is outside popup, destroy it
            if event.x_root < x1 or event.x_root > x2 or event.y_root < y1 or event.y_root > y2:
                popup.destroy()
                self.close_all_popups()  # This will also unbind

        # Remove previous binding if any
        if self.popup_close_binding:
            self.root.unbind('<Button-1>', self.popup_close_binding)
        self.popup_close_binding = self.root.bind('<Button-1>', check_click)
    ...
    def show_start_menu(self):
        ...
        self.start_menu.update_idletasks()  # Ensure geometry is updated
        start_x = self.start_btn.winfo_rootx()
        start_y = self.start_btn.winfo_rooty()
        popup_width = 400
        popup_height = 500
        # Center above start button (approx)
        x = start_x - popup_width // 2 + self.start_btn.winfo_width() // 2
        y = start_y - popup_height - 5
        # Ensure it stays on screen
        x = max(5, min(x, self.root.winfo_screenwidth() - popup_width - 5))
        y = max(5, y)
        self.start_menu.geometry(f"{popup_width}x{popup_height}+{x}+{y}")
        ...
        # --- Top bar: User + Power ---
        top_bar = tk.Frame(main, bg='black')
        top_bar.pack(fill=tk.X, pady=(0, 10))

        # User avatar (emoji) and name
        user_frame = tk.Frame(top_bar, bg='black')
        user_frame.pack(side=tk.LEFT)
        tk.Label(user_frame, text='👤', font=('Segoe UI', 16),
                 fg='#1E90FF', bg='black').pack(side=tk.LEFT, padx=(0,5))
        tk.Label(user_frame, text='User', font=('Segoe UI', 10),
                 fg='#1E90FF', bg='black').pack(side=tk.LEFT)

        # Power button
        tk.Label(top_bar, text='⏻', font=('Segoe UI', 14),
                 fg='#1E90FF', bg='black', cursor='hand2').pack(side=tk.RIGHT)

        # --- Search box (opens search popup on click) ---
        search_frame = tk.Frame(main, bg='#1E90FF', height=30)
        search_frame.pack(fill=tk.X, pady=(0, 10))
        search_frame.pack_propagate(False)
        search_label = tk.Label(search_frame, text='🔍  Search', font=('Segoe UI', 10),
                                fg='black', bg='#1E90FF', cursor='hand2')
        search_label.pack(expand=True, fill=tk.BOTH)
        # On click, open search popup and close start menu
        search_label.bind('<Button-1>', lambda e: [self.start_menu.destroy(), self.show_search()])

        # --- Pinned apps section ---
        tk.Label(main, text="Pinned", font=('Segoe UI', 10, 'bold'),
                 fg='#1E90FF', bg='black').pack(anchor='w', pady=(0,5))

        pinned_apps = [
            ("⚙️", "Settings"), ("📁", "File Explorer"), ("🛒", "Store"),
            ("🧮", "Calculator"), ("📝", "Notepad"), ("📧", "Mail"),
            ("🎵", "Spotify"), ("🎮", "Xbox")
        ]
        pinned_frame = tk.Frame(main, bg='black')
        pinned_frame.pack(fill=tk.X, pady=(0,10))

        # Arrange in a grid (4 columns)
        for i, (icon, name) in enumerate(pinned_apps):
            btn = tk.Button(pinned_frame, text=f"{icon} {name}", bg='black', fg='#1E90FF',
                            activebackground='gray', activeforeground='#1E90FF',
                            bd=0, padx=5, pady=5, anchor='w', justify='left')
            btn.grid(row=i//4, column=i%4, sticky='w', padx=5, pady=2)
            # On click, show a message (placeholder)
            btn.config(command=lambda n=name: self.show_message(n, f"Opening {n}"))

        # --- Recommended section ---
        tk.Label(main, text="Recommended", font=('Segoe UI', 10, 'bold'),
                 fg='#1E90FF', bg='black').pack(anchor='w', pady=(0,5))

        recommended = ["📄 Document.txt", "📸 Screenshot.png", "📊 Report.xlsx"]
        for item in recommended:
            btn = tk.Button(main, text=item, bg='black', fg='#1E90FF',
                            activebackground='gray', activeforeground='#1E90FF',
                            bd=0, padx=10, pady=5, anchor='w')
            btn.pack(fill=tk.X, pady=1)
            btn.config(command=lambda i=item: self.show_message(i, f"Opening {i}"))

        # Separator
        ttk.Separator(main, orient='horizontal').pack(fill=tk.X, pady=10)

        # All apps toggle button
        ...
        self.setup_outside_click_close(self.start_menu)
    ...
    def populate_all_apps(self):
        ...
        for app in all_apps_list:
            btn = tk.Button(scrollable_frame, text=app, bg='black', fg='#1E90FF',
                            ...
                            bd=0, padx=10, pady=5, anchor='w')
            btn.pack(fill=tk.X, pady=1)
            btn.config(command=lambda a=app: self.show_message(a, f"Opening {a}"))

    # --- Other popups (search, task view, widgets, action center, calendar) ---
    # (These are kept similar but updated to use the new close-on-outside-click)
    def show_search(self):
        ...
        self.setup_outside_click_close(self.search_popup)
    ...
    def show_task_view(self):
        ...
        self.setup_outside_click_close(self.taskview_popup)
    ...
    def show_widgets(self):
        ...
        self.setup_outside_click_close(self.widgets_popup)
    ...
    def show_action_center(self):
        ...
        self.setup_outside_click_close(self.action_center_popup)
    ...
    def show_calendar(self):
        ...
        self.setup_outside_click_close(self.calendar_popup)
